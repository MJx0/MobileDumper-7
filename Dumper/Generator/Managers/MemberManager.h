...
public:
	inline MemberIterator(const std::shared_ptr<class StructWrapper>& Str, const std::vector<UEProperty>& Mbr, const std::vector<PredefType>* const Predefs = nullptr, int32 StartIdx = 0x0, int32 PredefStart = 0x0)
	    : Struct(Str),
	      Members(Mbr),
	      PredefElements(Predefs),
	      CurrentIdx(StartIdx),
	      CurrentPredefIdx(PredefStart)
	{
...
private:
	/* bIsProperty */
	inline bool IsValidUnrealMemberIndex() const { return size_t(CurrentIdx) < Members.size(); }
	inline bool IsValidPredefMemberIndex() const { return PredefElements ? size_t(CurrentPredefIdx) < PredefElements->size() : false; }
...
private:
	...
	inline bool HasMorePredefMembers() const { return PredefElements ? size_t(CurrentPredefIdx) < PredefElements->size() : false; }
	inline bool HasMoreUnrealMembers() const { return size_t(CurrentIdx) < Members.size(); }
...
public:
	...
	bool HasPredefinedConstructor() const;
