...
{
	/*
	 * Only checking Child directly would miss an enum nested inside a container property (eg. the
	 * key of a TMap<SomeEnum, X>, or the element of a TArray<SomeEnum>/TSet<SomeEnum>) -- such an
	 * enum still needs a forward declaration wherever its package's structs/classes reference it,
	 * even though the *property itself* isn't an EnumProperty/ByteProperty. Reusing
	 * GetPropertyDependency here (already recurses through Array/Set/Map/Optional/Delegate
	 * properties for #include dependency tracking) avoids re-implementing that same traversal.
	 */
	for (UEProperty Child : Struct.GetProperties())
	{
		std::unordered_set<int32> Dependencies;
		...
		for (int32 DependencyIdx : Dependencies)
		{
			...
			if (DependencyObject.IsA(EClassCastFlags::Enum) && DependencyObject.GetPackageIndex() == RequiredPackageIdx)
				EnumsToForwardDeclare.emplace_back(DependencyObject.GetIndex(), bMarkAsClass);
		}
	}
}
...
void PackageManager::HandleCycles()
{
	...
	FindCycleCallbackType CleanedUpOnCycleFoundCallback = [&HandledPackages](const PackageManagerIterationParams& OldParams, const PackageManagerIterationParams& NewParams, bool bIsStruct) -> void
	{
		...
		{
			if (((Cycle.CurrentPackage == CurrentPackageIndex && Cycle.PreviousPacakge == PreviousPackageIndex) || (Cycle.CurrentPackage == PreviousPackageIndex && Cycle.PreviousPacakge == CurrentPackageIndex)) && (Cycle.bAreStructsCyclic == bIsStruct || Cycle.bAreclassesCyclic == !bIsStruct))
			{
				return;
			}
		}
		...
		if (bIsStruct)
		{
			bIsMutualInclusion = CurrentPackageInfo.GetPackageDependencies().StructsDependencies.contains(PreviousPackageIndex) && PreviousPackageInfo.GetPackageDependencies().StructsDependencies.contains(CurrentPackageIndex);
		}
		else
		{
			bIsMutualInclusion = CurrentPackageInfo.GetPackageDependencies().ClassesDependencies.contains(PreviousPackageIndex) && PreviousPackageInfo.GetPackageDependencies().ClassesDependencies.contains(CurrentPackageIndex);
		}
	...
	for (const CycleInfo& Cycle : HandledPackages)
	{
		 ...
		 * bIsClass must match which dependency graph/file actually lost the include below (structs-cyclic -> the _structs.hpp file), not
		 * Cycle.bAreStructsCyclic directly -- that would tag the forward declaration for the wrong file (_classes.hpp) whenever the cycle
		 * was found in the *structs* graph, leaving the *structs* file (the one that actually needs it) without the enum declared at all.
		 */
		HelperInitEnumFwdDeclarationsForPackage(Cycle.CurrentPackage, Cycle.PreviousPacakge, Cycle.bAreclassesCyclic);
		...
		auto Dependencies = CurrentPackageInfo.GetPackageDependencies().ClassesDependencies.find(Cycle.PreviousPacakge);
		if (Dependencies == CurrentPackageInfo.GetPackageDependencies().ClassesDependencies.end())
		{
			GLogger.FmtWrite(ELogLevel::Warning, "Warning PackageManager::HandleCycles(): Pacakge({}) not in ClassesDependencies!", Cycle.PreviousPacakge);
			continue;
		}

		const RequirementInfo& CurrentRequirements = Dependencies->second;
