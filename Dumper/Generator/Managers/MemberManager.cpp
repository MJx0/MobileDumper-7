...
{
	// A struct's raw reflected size can still exceed how it's actually laid out when embedded
	// elsewhere (see StructManager's cross-struct size validation) - drop whatever no longer fits
	const int32 CorrectedSize = StructManager::GetInfo(Str).GetUnalignedSize();
	Members.erase(std::remove_if(Members.begin(), Members.end(), [CorrectedSize](UEProperty Prop)
	{
		const int32 ArrayDim = Prop.GetArrayDim();
		return ArrayDim <= 0x0 || (Prop.GetOffset() + (Prop.GetSize() * ArrayDim)) > CorrectedSize;
	}),
	              Members.end());
...
bool MemberManager::HasPredefinedConstructor() const
{
	if (!PredefFunctions)
		return false;

	for (const PredefinedFunction& Func : *PredefFunctions)
	{
		if (Func.bIsCtor)
			return true;
	}
