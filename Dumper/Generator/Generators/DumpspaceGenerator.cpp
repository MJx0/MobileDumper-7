...
std::string DumpspaceGenerator::GetMemberTypeStr(UEProperty Property, std::string& OutExtendedType, std::vector<DSGen::MemberType>& OutSubtypes)
{
	...
	else if (Flags & EClassCastFlags::StructProperty)
	{
		...
		return CppGeneratorAccessor::GetStructPrefixedName(UnderlayingStruct);
	}
	...
	else if (Flags & EClassCastFlags::ObjectProperty)
	{
		...
		if (UEClass PropertyClass = Member.Cast<UEObjectProperty>().GetPropertyClass())
			return CppGeneratorAccessor::GetStructPrefixedName(PropertyClass);

		return "UObject";
	}
	else if (GSettings.EngineCore.bEnableEncryptedObjectPropertySupport && Flags & EClassCastFlags::ObjectPropertyBase && Member.GetSize() == 0x10)
	{
		if (UEClass PropertyClass = Member.Cast<UEObjectProperty>().GetPropertyClass())
			return fmt::format("TEncryptedObjPtr<class {}>", CppGeneratorAccessor::GetStructPrefixedName(PropertyClass));
	...
	else if (Flags & EClassCastFlags::MapProperty)
	{
		...
		OutSubtypes.emplace_back(GetMemberType(MemberAsMapProperty.GetKeyProperty()));
		OutSubtypes.emplace_back(GetMemberType(MemberAsMapProperty.GetValueProperty()));
...
DSGen::MemberType DumpspaceGenerator::GetMemberType(const StructWrapper& Struct)
{
	...
	Type.typeName     = CppGeneratorAccessor::GetStructPrefixedName(Struct);
...
DSGen::ClassHolder DumpspaceGenerator::GenerateStruct(const StructWrapper& Struct)
{
	...
	StructOrClass.className      = CppGeneratorAccessor::GetStructPrefixedName(Struct);
...
DSGen::EnumHolder DumpspaceGenerator::GenerateEnum(const EnumWrapper& Enum)
{
	...
	Enumerator.enumName = CppGeneratorAccessor::GetEnumPrefixedName(Enum);
...
{
	DSGen::addOffset("MobileDumper", 7);

	DSGen::addOffset("OFFSET_GOBJECTS", GInSDKOffsets.Statics.GObjects);
	DSGen::addOffset("OFFSET_GNAMES", GInSDKOffsets.Statics.GNames);
	DSGen::addOffset("OFFSET_GWORLD", GInSDKOffsets.Statics.GWorld);
	DSGen::addOffset("OFFSET_ProcessEvent", GInSDKOffsets.Statics.PEOffset);
	DSGen::addOffset("INDEX_ProcessEvent", GInSDKOffsets.Statics.PEIndex);
}

void DumpspaceGenerator::Generate()
{
	/* Set the output directory of DSGen to "...GenerationPath/Dumespace" */
	...
	for (PackageInfoHandle Package : PackageManager::IterateOverPackageInfos())
	{
		...
		{
			/* The object at EnumIdx can go stale between Package's enum-list being built and
			 * this loop running, since the target is a live, running game whose object array
			 * keeps mutating while we dump it. */
			UEEnum Enum = ObjectArray::GetByIndex<UEEnum>(EnumIdx);
			if (!Enum)
				continue;

			DSGen::EnumHolder GeneratedEnum = GenerateEnum(Enum);
			DSGen::bakeEnum(GeneratedEnum);
		}
