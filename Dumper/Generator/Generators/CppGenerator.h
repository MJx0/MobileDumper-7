...
private:
	...
	static std::string MakeEnumMemberString(const std::string& Name, const std::string& Value, const std::string& Underlyingtype, bool bIsOutOfRange);
...
private: /* utility functions */
	static int32 FindFreeOffset(const std::vector<PredefinedMember>& Properties, int32 Size, int32 Alignment);
...
{
public:
	inline static std::string GetStructPrefixedName(const StructWrapper& Struct)
	{
		return CppGenerator::GetStructPrefixedName(Struct);
	}

	inline static std::string GetEnumPrefixedName(const EnumWrapper& Enum)
	{
		return CppGenerator::GetEnumPrefixedName(Enum);
	}

	inline static std::string GetMemberTypeString(const PropertyWrapper& Property)
	{
