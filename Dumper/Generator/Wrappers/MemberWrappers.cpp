...
PropertyWrapper::PropertyWrapper(const std::shared_ptr<StructWrapper>& Str, UEProperty Prop)
    : Property(Prop),
      Struct(Str),
      Name(MemberManager::GetNameCollisionInfo(Str->GetUnrealStruct(), Prop)),
...
FunctionWrapper::FunctionWrapper(const std::shared_ptr<StructWrapper>& Str, UEFunction Func)
    : Function(Func),
      Struct(Str),
      Name(Str ? MemberManager::GetNameCollisionInfo(Str->GetUnrealStruct(), Func) : NameInfo()),
      bIsUnrealFunction(true)
{
}
...
std::string FunctionWrapper::GetParamStructName() const
{
	...
	return Struct->GetUniqueName().first + "_" + GetName();
}
...
std::string FunctionWrapper::GetPredefFunctionInlineBody() const
{
	...
	for (size_t i = 0; i < BodyCopy.size(); i++)
	{
		if (BodyCopy[i] == '\n')
			BodyCopy.insert(++i, "\t");
	}
...
uintptr_t FunctionWrapper::GetExecFuncOffset() const
{
	...
	return GMemory->GetUnrealModule().AddressToOffset(reinterpret_cast<uintptr_t>(Function.GetExecFunction()));
}
...
bool FunctionWrapper::IsConstructor() const
{
	return !bIsUnrealFunction && PredefFunction->bIsCtor;
}
